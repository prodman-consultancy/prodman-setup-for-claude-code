...
Run it on the strongest setting you have: send `/model opus` and `/effort max` before you start.
The runbook checks both before it touches the machine and asks you to fix them if they are lower,
because this is a run that installs software and writes configuration, not a chat.
